  ...
  function create(options) {
    ...
    function creatureCard(creature, profile, compact) {
      ...
      return `<article class="ix-creature ${compact ? 'is-compact' : ''}" data-rarity="${species.rarity}">
        ...
        <small>${escapeHtml(species.name)} · ${escapeHtml(Data.rarity[species.rarity].label)}</small>
        ...
        ${compact ? '' : `<div class="ix-xp"><i style="width:${Math.min(100,Math.round(((creature.xp || 0) / Math.max(1,neededXp))*100))}%"></i><span>경험치 ${creature.xp || 0}/${neededXp}</span></div><div class="ix-card-actions"><button data-act="detail" data-id="${escapeHtml(creature.id)}">${expanded ? '정보 닫기' : '상세 정보'}</button><button data-act="team" data-id="${escapeHtml(creature.id)}">${inTeam ? '팀 해제' : '팀 편성'}</button><button data-act="feed" data-id="${escapeHtml(creature.id)}" ${(profile.supplies && profile.supplies.feeds || 0) < 1 ? 'disabled' : ''}>사료 주기</button>${species.evolvesTo ? `<button data-act="evolve" data-id="${escapeHtml(creature.id)}" ${(creature.level || 1) < 5 ? 'disabled' : ''}>${(creature.level || 1) < 5 ? 'Lv.5 진화' : '진화하기'}</button>` : ''}</div>${expanded ? `<div class="ix-creature-detail"><button data-act="rename" data-id="${escapeHtml(creature.id)}">이름 바꾸기</button><div class="ix-detail-tags"><span>${escapeHtml(species.stage)}</span><span>${escapeHtml(species.habitat)}</span><span>포획 난도 ${species.captureDifficulty}</span></div><dl><div><dt>생명력</dt><dd>${stats.maxHealth || maxHp}</dd></div><div><dt>공격</dt><dd>${stats.attack || stat(creature,'attack')}</dd></div><div><dt>방어</dt><dd>${stats.defense || stat(creature,'defense')}</dd></div><div><dt>속도</dt><dd>${stats.speed || stat(creature,'speed')}</dd></div></dl><p class="ix-feed-note">사료 1개 = 경험치 +84.${species.evolvesTo ? ` Lv.5부터 ${escapeHtml(Data.speciesById[species.evolvesTo].name)}로 진화하며 공격도 바뀝니다.` : ' 이 곤충은 레벨을 올려 능력치를 키울 수 있어요.'}</p><div class="ix-move"><b>${escapeHtml(species.normalAttack.name)}</b><span>위력 ${species.normalAttack.power} · 명중 ${Math.round(species.normalAttack.accuracy*100)}%</span></div>${species.skill ? `<div class="ix-move is-skill"><b>${escapeHtml(species.skill.name)}</b><span>위력 ${species.skill.power} · 재사용 ${species.skill.cooldown}턴</span><p>${escapeHtml(species.skill.description)}</p></div>` : '<p class="ix-no-skill">일반종은 고유 기술 없이 기본 공격으로 싸웁니다.</p>'}</div>` : ''}`}
      </article>`;
    }
    ...
    function characterScreen() {
      ...
      return `<section class="ix-onboard" aria-label="탐험가 선택"><div class="ix-onboard-copy"><p class="ix-eyebrow">공유 세계 곤충 탐험</p><h1>${Data.title}</h1><p>서로 다른 서식지를 누비며 작은 생명들과 팀을 이루세요.</p></div>
        ...
        <button id="enter-world" class="ix-enter" data-act="enter" data-action="enter">이슬숲으로 출발</button></section>`;
    }

    function topBar(profile) {
      const online = listOf(snapshot.players).length;
      return `<header class="ix-top"><div><p class="ix-eyebrow">현재 지역</p><strong>${escapeHtml(snapshot.locationName || '해오름 초원')}</strong></div><div class="ix-online"><i></i>${online}명 탐험 중</div><span class="ix-feed" title="곤충 사료">🍀 ${profile.supplies && profile.supplies.feeds || 0}</span><button data-act="heal" data-action="heal" title="팀 전체 회복">회복</button><button data-act="sound" title="효과음 켜기·끄기">소리</button><button class="ix-exit" data-act="exit" title="대기실로 돌아가기">나가기</button></header>`;
    }
    ...
    function nearbySpawn(state) {
      ...
      const candidates = listOf(state.spawns).filter(s => s.available !== false && !s.reservedBy)
        ...
        .filter(({spawn, distance}) => distance <= (spawn.field ? 7 : 4) && reachable(spawn, distance))
    ...
    function nearbyPanel() {
      ...
      return `<div class="ix-nearby" aria-label="가까운 곤충"><span>${escapeHtml(species.name)}</span><button id="selected-${action}" data-act="${action}" data-action="${action}" data-id="${escapeHtml(spawn.id)}">${spawn.field ? '⚔ 전투 요청' : '✦ 채집하기'}</button></div>`;
    }
    ...
    function selectionPanel() {
      ...
      if (selection.type === 'npc' && selection.id === 'guide-mira') {
        ...
        const text = quest.status === 'available' ? '첫 의뢰가 있어요. 곤충 3마리를 채집해 볼까요?' : quest.status === 'active' ? `채집 기록 ${quest.progress}/${quest.target}. 초원을 둘러보세요.` : quest.status === 'ready' ? '채집 완료! 사료 보상을 받아 가세요.' : '다음 채집 의뢰가 준비됐어요. 계속 도와줄래요?';
        return `<div class="ix-selection ix-npc"><span class="ix-selection-icon">🧑‍🔬</span><div><small>이슬숲 연구소</small><strong>미라 연구원</strong><span>${text}</span></div><button data-act="quest">대화하기</button></div>`;
      }
    ...
    function drawer(profile) {
      ...
      if (panel === 'collection') return `<aside class="ix-drawer"><div class="ix-drawer-head"><div><small>곤충 도감</small><h2>내 탐험대 <b>${collection.length}</b></h2></div><button data-act="close-panel" aria-label="닫기">×</button></div><div class="ix-drawer-scroll">${collection.length ? collection.map(c => creatureCard(c, profile, false)).join('') : '<div class="ix-empty"><span>🍃</span><strong>첫 곤충을 만나 보세요</strong><p>야생 곤충을 선택해 포획을 시도할 수 있어요.</p></div>'}</div></aside>`;
      if (panel === 'map') return `<aside class="ix-drawer ix-map-drawer"><div class="ix-drawer-head"><div><small>사냥터 이동</small><h2>탐험 지도</h2></div><button data-act="close-panel" aria-label="닫기">×</button></div><p class="ix-map-copy">원하는 지역을 누르면 해당 사냥터 입구로 이동합니다. 지역마다 만나는 곤충과 레벨이 달라요.</p><div class="ix-map-list">${Data.biomes.map(b => `<button data-act="travel" data-id="${escapeHtml(b.id)}" class="${snapshot.locationName === b.name ? 'is-here' : ''}"><i style="background:${escapeHtml(b.color)}"></i><span><b>${escapeHtml(b.name)}</b><small>${b.safe ? '안전한 마을 · 회복과 의뢰' : `Lv.${regionLevels(b)} · ${escapeHtml(b.habitat || '사냥터')}`}</small></span><em>${b.safe ? '마을' : '이동'}</em></button>`).join('')}</div></aside>`;
      const known = new Set(profile.discoveries || []);
      return `<aside class="ix-drawer"><div class="ix-drawer-head"><div><small>발견 기록</small><h2>이슬숲 도감 <b>${known.size}/${Data.species.length}</b></h2></div><button data-act="close-panel" aria-label="닫기">×</button></div><div class="ix-encyclopedia">${Data.species.map(s => `<div class="${known.has(s.id) ? '' : 'is-unknown'}"><span>${known.has(s.id) ? modelFor(s.id, s.category) : '?'}</span><strong>${known.has(s.id) ? escapeHtml(s.name) : '미발견'}</strong><small>${known.has(s.id) ? escapeHtml(s.habitat) : '탐험 필요'}</small></div>`).join('')}</div></aside>`;
    }
    ...
    function battleModal(profile) {
      ...
      const captureText = capture && capture.success ? `${escapeHtml(Data.speciesById[capture.speciesId] && Data.speciesById[capture.speciesId].name || '필드 곤충')} 포획 성공` : capture && capture.failureReason === 'inventory-full' ? '보관함이 가득 차 포획 보상을 받지 못했습니다.' : capture && capture.success === false ? `포획 실패 · 성공 확률 ${Math.round((capture.chance || 0) * 100)}%` : '';
      const rewardText = [outcome && outcome.totalXp > 0 ? `총 경험치 +${outcome.totalXp} · 참여 곤충당 ${outcome.xpPerCreature}` : '', captureText].filter(Boolean).join(' · ') || '전투 기록이 안전하게 저장됩니다.';
      ...
      return `<section class="ix-battle" aria-label="턴제 전투"><header><div><small>${battle.type === 'pvp' ? '탐험가 대전 · 3 대 3' : '필드 전투 · 3 대 1'}</small><strong>턴 ${battle.turn}</strong></div><div class="ix-turn"><i style="--time:${Math.min(25, time)}"></i><b>${time}</b>초</div></header>
        <div class="ix-battlefield"><div class="ix-fighter is-enemy"><div class="ix-fighter-info"><strong>${escapeHtml(foeActive.nickname || foeSpecies.name)}</strong><span>Lv.${foeActive.level}</span><div class="ix-hp"><i style="width:${hpPercent(foeActive)}%"></i></div><small>${foeActive.hp}/${foeActive.maxHp}</small></div></div>
        <div class="ix-impact" aria-live="polite">${lastHit ? `${lastHit.critical ? '치명타 · ' : ''}-${lastHit.amount}` : '대치 중'}</div><div class="ix-fighter is-player"><div class="ix-fighter-info"><strong>${escapeHtml(yourActive.nickname || species.name)}</strong><span>Lv.${yourActive.level} · 전투력 ${yourActive.cp || yourActive.combatPower || '—'}</span><div class="ix-hp"><i style="width:${hpPercent(yourActive)}%"></i></div><small>${yourActive.hp}/${yourActive.maxHp}</small></div></div></div>
        ...
        ${battle.status === 'finished' ? `<div class="ix-battle-result"><strong>${outcome && outcome.winner === youKey ? '탐험 승리' : outcome && outcome.reason === 'retreat' ? '안전하게 후퇴했습니다' : '전투 종료'}</strong><span>${rewardText}${levelText}</span></div><button class="ix-return" data-act="return" data-action="return">탐험지로 돌아가기</button>` : `<div class="ix-actions"><button data-act="battle-action" data-action="attack" data-value="attack"><span>⚔️</span><strong>${escapeHtml(species.normalAttack.name)}</strong><small>일반 공격</small></button>${species.skill ? `<button data-act="battle-action" data-action="skill" data-value="skill" ${(yourActive.cooldowns && yourActive.cooldowns[species.skill.id]) > 0 ? 'disabled' : ''}><span>✦</span><strong>${escapeHtml(species.skill.name)}</strong><small>${(yourActive.cooldowns && yourActive.cooldowns[species.skill.id]) || '고유 기술'}</small></button>` : ''}<button data-act="open-switch" data-action="switch-menu"><span>↻</span><strong>교체</strong><small>남은 ${mine.team.filter(c => c.hp > 0).length}마리</small></button>${battle.type === 'field' ? `<button data-act="battle-action" data-action="capture" data-value="capture"><span>◉</span><strong>포획</strong><small>체력 30% 이하</small></button><button data-act="battle-action" data-action="retreat" data-value="retreat"><span>⌂</span><strong>도주</strong><small>탐험지 복귀</small></button>` : ''}</div>`}
    ...
    function render() {
      ...
      const questText = quest.status === 'available' ? '미라 연구원의 첫 의뢰를 받아 보세요.' : quest.status === 'active' ? `채집 의뢰 · 곤충 ${quest.progress}/${quest.target}` : quest.status === 'ready' ? '첫 의뢰 완료 · 연구원에게 돌아가세요!' : '사료 +3 획득! 다음 채집 의뢰 받기';
      root.innerHTML = `${topBar(profile)}<nav class="ix-nav"><button class="${panel === 'collection' ? 'is-active' : ''}" data-act="panel" data-value="collection"><span>🪲</span>곤충</button><button class="${panel === 'encyclopedia' ? 'is-active' : ''}" data-act="panel" data-value="encyclopedia"><span>▦</span>도감</button><button class="${panel === 'map' ? 'is-active' : ''}" data-act="panel" data-value="map"><span>🗺️</span>지도</button></nav>
        <div class="ix-team" aria-label="현재 팀"><small>전투 팀 ${team.length}/3</small>${team.map(c => `<span title="${escapeHtml(c.nickname || speciesOf(c).name)}">${modelFor(c.speciesId, speciesOf(c).category)}</span>`).join('')} ${team.length ? '' : '<b>곤충을 팀에 편성하세요</b>'}</div>
        <button class="ix-quest-strip" data-act="quest-guide">📜 ${escapeHtml(questText)}</button>${nearbyPanel()}${selectionPanel()}${drawer(profile)}${challengeModal()}${battleModal(profile)}<div class="ix-toast ${toastState ? 'is-visible' : ''}" data-tone="${toastState ? toastState.tone : 'info'}" role="status">${toastState ? escapeHtml(toastState.message) : ''}</div>`;
    }
    ...
    function signature(state) {
      ...
      return JSON.stringify({
        ...
        profile: { characterId: profile.characterId, team: profile.team, collection: profile.collection, discoveries: profile.discoveries, supplies: profile.supplies, quest: profile.quest },
        players: listOf(state.players).map(p => [p.uid || p.id, p.name || p.nickname, Boolean(p.busy), p.characterId]),
    ...
    root.addEventListener('click', async event => {
      ...
      if (act === 'enter') { const result = await command('character', { id: chosenCharacter }); if (result === null || (result && result.ok === false)) return; entered = true; if (options.onEnter) options.onEnter(chosenCharacter); render(); return; }
      if (act === 'panel') { panel = panel === button.dataset.value ? null : button.dataset.value; render(); return; }
      ...
      if (act === 'quest-guide') {
        ...
        if (me && Math.hypot(me.x - 7, me.z - 11) <= 4) return command('quest', {});
        const result = await command('travel', { biomeId: 'safe' });
        if (result) { selection = { type: 'npc', id: 'guide-mira' }; panel = null; render(); }
        return;
      }
      if (act === 'quest') return command('quest', {});
