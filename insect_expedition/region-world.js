(function(global){
  ...
  function create(scene,parent,helpers){
    ...
    function setRegion(id){
      ...
      const groundMat=groundMaterial(scene,{id:rocky?'rock':id,color:theme.ground},7300);groundMat.diffuseColor=B.Color3.FromHexString(theme.ground);groundMat.specularColor=B.Color3.Black();
      if(groundMat.diffuseTexture)groundMat.diffuseTexture.uScale=groundMat.diffuseTexture.vScale=38;
      const terrain=B.MeshBuilder.CreateGround('region-terrain',{width:430,height:430,subdivisions:110,updatable:true},scene);terrain.parent=root;terrain.material=groundMat;terrain.receiveShadows=true;
      ...
      for(let i=0;i<(sparse?35:105);i++){
        ...
        if(Math.abs(x)<12&&z>0||R.portals(id).some(p=>Math.hypot(p.x-x,p.z-z)<10))continue;
        if(sparse){
      ...
      for(const o of R.obstacles(id))if(o.type==='circle')ball(o.id,o.x,3,o.z,o.radius*2,9,o.radius*2,stone);
      ...
      terrainStrip('hill-trail',z=>id==='safe'?0:Math.sin(z*.025)*9,5.5,-135,100,path);
      if(id==='safe'){
    ...
    function applyAtmosphere(){const t=R.themes[current]||R.themes.safe;scene.clearColor=B.Color4.FromHexString(t.sky+'ff');scene.fogColor=B.Color3.FromHexString(t.sky);scene.fogStart=['forest','wetland','cave','mine','nest'].includes(current)?65:115;scene.fogEnd=210;}
    function frame(now){gates.forEach((g,i)=>{g.ring.rotation.z=Math.sin(now/1200+i)*.08;g.surface.visibility=.25+Math.sin(now/450+i)*.1;});}
