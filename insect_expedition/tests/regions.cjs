...
test('포탈 거리·지역 격리·판매 수량·전투 뒤 달리기·영구 저장',async()=>{
  ...
  try{
    ...
    assert.equal((await a.command('portal',{portalId:'portal-forest'})).ok,false);
    Object.assign(p,R.portals('safe').find(p=>p.to==='grassland')); // Only position is relevant to entry.
    const enter=await a.command('portal',{portalId:'portal-grassland'});assert(enter.ok,enter.error);assert.equal(p.regionId,'grassland');assert.equal(a.state().players.length,1);assert(a.state().spawns.every(s=>s.biomeId==='grassland'));
